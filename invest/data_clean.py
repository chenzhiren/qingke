...
def biao(data):
    ...
    data_in = data[data['地区'] == '国内']
    data_out = data[data['地区'] == '国外']
    l1_data = pd.DataFrame(data_in['数据分层'].value_counts())
    pie = (Pie()
           .add('', [list(z) for z in zip(l1_data.index.to_list(), l1_data['数据分层'].to_list())])
           .set_global_opts(title_opts=opts.TitleOpts(title="数据比例"))
           .set_series_opts(label_opts=opts.LabelOpts(formatter="{b}: {c}\n 占比{d}%"))

           )
    return data_in, data_out, pie.render_notebook()
