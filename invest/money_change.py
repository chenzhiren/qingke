        ...
        for x in currency:
            if x in a:
                if x=='暂不处理':
                   ...
                   data.iloc[i,13] = ''.join(list(data.iloc[i,8])[0:-3])
                else:
                   data.iloc[i, 13] = ''.join(list(data.iloc[i,8])[0:-2])
    data=data.drop(['money'],axis=1).rename(columns={'next_money':'money'})
    mid=data.pop('money')
    data.insert(8,'money',mid)
    data['money']=data['money'].apply(quza)
    return data
